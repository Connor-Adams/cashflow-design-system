...
export function Progress({ value = 0, tone = 'primary', size = 'default', indeterminate = false, label, showValue = false, className, style, ...props }) {
    ...
    <div data-slot="progress" className={className} style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%', ...style }} {...props}>
      {(label || showValue) && (
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--text-body-sm)', fontFamily: 'var(--font-sans)' }}>
          ...
          {showValue && !indeterminate && <span style={{ color: 'var(--foreground)', fontFamily: 'var(--font-mono)', fontWeight: 'var(--weight-semibold)' }}>{Math.round(pct)}%</span>}
        </div>
      )}
      <div
        role="progressbar"
