...
export function Combobox({ options = [], value, defaultValue = null, onValueChange, placeholder = 'Search…', emptyText = 'No matches', size = 'default', className, style, ...props }: ComboboxProps): React.JSX.Element {
    ...
    <div ref={ref} data-slot="combobox" className={className} style={{ position: 'relative', display: 'inline-block', minWidth: 220, fontFamily: 'var(--font-sans)', ...style }} {...props}>
      ...
      >
        ...
        <input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setQuery(e.target.value); if (!open) setOpen(true) }}
